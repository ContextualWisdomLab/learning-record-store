...
The controlled writers accept the reviewed protocol/comparison pairs: received `2.0` or `2.0.0` with `xapi-2.0-statement-comparison/v1`, or `1.0.3` with `xapi-1.0.3-statement-comparison/v1`. `ingestion_receipt` preserves the exact received label. `statement_record` stores canonical `2.0.0` for either xAPI 2.0 input, preventing an alias-only replay conflict. An unknown protocol label or a cross-version comparison label fails before receipt or canonical mutation.
