...
The first executable migration implements `tenant_partition`, `ingestion_receipt`, `statement_ingestion_item`, `statement_record`, and `voiding_relation`. It also defines `persist_statement_occurrence`, the item-level durable transaction primitive that records a request receipt, resolves one canonical Statement identity through the primary-key conflict path, compares retained version/comparison evidence, and records the accepted/replayed/conflict occurrence before returning. The remaining entities are planned protocol surfaces and must not be represented as implemented until migrations and tests exist.
