...
The PR head's executable migration set currently implements `tenant_partition`, `tenant_database_principal`, `ingestion_receipt`, `statement_ingestion_item`, `statement_record`, and `voiding_relation`. It also defines `authorized_tenant_key()`, `statement_advisory_lock_key()`, `statement_comparison_version_for_xapi()`, `persist_statement_occurrence`, the Proposed `persist_statement_batch` primitive, and a serialized trigger guard that prevents a Statement from occupying both voiding roles. None is an approved release contract until the stack is integrated through the protected branch. The remaining entities are planned protocol surfaces and must not be represented as implemented until migrations and tests exist.
...
A validated voiding Statement retains its parsed StatementRef target as immutable semantic evidence alongside its canonical bytes. Replay equality includes this target: an ordinary Statement cannot later be reclassified as voiding, and a voiding Statement cannot change its target. `VoidingRelation` is derived only from that retained meaning; callers cannot nominate an unrelated target.
...
The controlled writers accept the reviewed protocol/comparison pairs: received `2.0` or `2.0.0` with `xapi-2.0-statement-comparison/v1`, or `1.0` and valid `1.0.x` labels with `xapi-1.0.3-statement-comparison/v1`. `ingestion_receipt` preserves the exact received label. `statement_record` stores canonical `2.0.0` for either xAPI 2.0 input and canonical `1.0.0` for the stable xAPI 1.0 data-model surface, preventing alias-only replay conflicts. Unknown, malformed (including leading-zero patch forms), or cross-version protocol/comparison pairs fail before receipt or canonical mutation.
