...
The SQL functions trust an upstream adapter to supply a validated header value. The Rust value object now enforces header presence/cardinality, UTF-8 encoding, and supported exact labels, but response header selection, version negotiation outside these bounded mappings, durable receipt adaptation, JSON validation, full Statement comparison, cmi5 behavior, and protocol error mapping remain unimplemented. Treating this slice as full xAPI conformance would be false assurance.
