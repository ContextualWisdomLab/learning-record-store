...
These paths may be cited as implementation evidence only after the exact PR head passes the corresponding hosted quality workflow. `tests/protocol_version_header.rs` exercises bounded header cardinality/encoding extraction plus request/response-version selection, not an HTTP endpoint or standards conformance. Proposed migration 0004 and `tests/postgres_batch_transaction.sh` provide the internal durable one-receipt/many-item PostgreSQL primitive and transaction evidence. They do not change the `Not implemented` conformance maturity above because the durable Rust/REST adapter, response-header emission, Statement JSON parser/comparator, attachments, document resources, and independent xAPI/cmi5 conformance suites are still absent. ADR 0002 records the authorization decision; `docs/product-technical-gap-baseline.md` records the remaining commercialization verification order.
