...
`tests/voiding_invariants.rs` proves only the repository's internal content-binding policy: a validator-classified StatementRef target is retained with the immutable Statement, ordinary Statements cannot authorize voiding, and callers cannot choose a different relation target. It is not xAPI or cmi5 conformance evidence. `tests/ingestion_kernel.rs`, `tests/postgres_principal_boundary.sh`, and `tests/postgres_batch_transaction.sh` prove only the normalized/durable version-label boundary: received `2.0` is retained on its receipt, canonicalized to `2.0.0` on the Statement, and replays through exact `2.0.0`; received `1.0` and valid `1.0.x` labels remain exact on receipts, canonicalize to `1.0.0` in both the Rust kernel and PostgreSQL Statement record, and replay on the reviewed xAPI 1.0.3 comparison surface. Malformed leading-zero patch forms fail closed with SQLSTATE `22023`, the expected incompatibility message, and no receipt, occurrence, or canonical Statement across item and multi-item batch calls. The future version-specific HTTP parser/adapter and independent conformance suite must prove header presence, response-version behavior, broader version rules, StatementRef semantics, and protocol error mapping before this row can advance.
