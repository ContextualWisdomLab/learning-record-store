...
Primary-source status was rechecked on 2026-09-19. IEEE 9274.1.1-2023 remains an active xAPI base standard, and ISO/IEC/IEEE 39274-1-1:2025 remains the published international adoption. cmi5 Quartz remains a separate xAPI 1.0.3 compatibility surface; it is not evidence that an xAPI 2.0 implementation conforms to cmi5. The IEEE LRS requirements explicitly require accepting `X-Experience-API-Version: 2.0` as `2.0.0`. ADL xAPI 1.0.3 likewise requires `1.0` to be processed as `1.0.0` and otherwise-valid `1.0.x` headers to be accepted. The Rust `ReceivedXapiVersion` boundary requires exactly one UTF-8 header value, preserves one exact accepted label while selecting its normalized surface, and fails closed on missing or ambiguous input. The PostgreSQL boundary retains that exact label on the immutable receipt while normalizing canonical Statement processing to `2.0.0` or stable xAPI 1.0 data-model label `1.0.0`; this is bounded extraction/persistence evidence, not full HTTP or conformance evidence.
...
These paths may be cited as implementation evidence only after the exact PR head passes the corresponding hosted quality workflow. `tests/protocol_version_header.rs` exercises bounded header cardinality/encoding extraction and request-version selection, not an HTTP endpoint or standards conformance. Proposed migration 0004 and `tests/postgres_batch_transaction.sh` provide the internal durable one-receipt/many-item PostgreSQL primitive and transaction evidence. They do not change the `Not implemented` conformance maturity above because the durable Rust/REST adapter, response negotiation, Statement JSON parser/comparator, attachments, document resources, and independent xAPI/cmi5 conformance suites are still absent. ADR 0002 records the authorization decision; `docs/product-technical-gap-baseline.md` records the remaining commercialization verification order.

`tests/voiding_invariants.rs` proves only the repository's internal content-binding policy: a validator-classified StatementRef target is retained with the immutable Statement, ordinary Statements cannot authorize voiding, and callers cannot choose a different relation target. It is not xAPI or cmi5 conformance evidence. `tests/protocol_version_header.rs`, `tests/ingestion_kernel.rs`, `tests/postgres_principal_boundary.sh`, and `tests/postgres_batch_transaction.sh` prove only the normalized/durable version-label boundary: exactly one UTF-8 `2.0`/`2.0.0` value selects canonical `2.0.0`; exactly one UTF-8 `1.0` or syntactically valid `1.0.x` value selects stable `1.0.0` and the reviewed xAPI 1.0.3 comparison implementation. Missing, repeated, non-UTF-8, whitespace-altered, combined, malformed, leading-zero patch, and unsupported values fail closed in Rust; malformed persistence inputs fail with SQLSTATE `22023`, the expected incompatibility message, and no receipt, occurrence, or canonical Statement across item and multi-item batch calls. The future adapter and independent conformance suite must prove durable receipt mapping, response-version behavior, Statement JSON/comparison rules, StatementRef semantics, and protocol error mapping before this row can advance.
