...
These paths may be cited as implementation evidence only after the exact PR head passes the corresponding hosted quality workflow. `tests/protocol_version_header.rs` exercises the bounded request-version value object, not HTTP extraction or standards conformance. Proposed migration 0004 and `tests/postgres_batch_transaction.sh` provide the internal durable one-receipt/many-item PostgreSQL primitive and transaction evidence. They do not change the `Not implemented` conformance maturity above because the Statement JSON parser/comparator, Rust/REST adapter, attachments, document resources, and independent xAPI/cmi5 conformance suites are still absent. ADR 0002 records the authorization decision; `docs/product-technical-gap-baseline.md` records the remaining commercialization verification order.

`tests/voiding_invariants.rs` proves only the repository's internal content-binding policy: a validator-classified StatementRef target is retained with the immutable Statement, ordinary Statements cannot authorize voiding, and callers cannot choose a different relation target. It is not xAPI or cmi5 conformance evidence. `tests/protocol_version_header.rs`, `tests/ingestion_kernel.rs`, `tests/postgres_principal_boundary.sh`, and `tests/postgres_batch_transaction.sh` prove only the normalized/durable version-label boundary: one exact `2.0`/`2.0.0` value selects canonical `2.0.0`; one exact `1.0` or syntactically valid `1.0.x` value selects stable `1.0.0` and the reviewed xAPI 1.0.3 comparison implementation. Whitespace-altered, combined, malformed, leading-zero patch, and unsupported values fail closed in Rust; malformed persistence inputs fail with SQLSTATE `22023`, the expected incompatibility message, and no receipt, occurrence, or canonical Statement across item and multi-item batch calls. The future HTTP adapter and independent conformance suite must prove header presence/cardinality, response-version behavior, Statement JSON/comparison rules, StatementRef semantics, and protocol error mapping before this row can advance.
