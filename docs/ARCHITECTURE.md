...
- `xapi_protocol`: request and response contracts. `ReceivedXapiVersion` extracts exactly one UTF-8 request-version header value, carries that value into in-memory receipt creation while selecting a normalized Statement/comparison surface and its latest supported response value, and fails closed on missing or repeated values; the durable repository adapter and HTTP response emission remain separate.
...
- `statement_validation`: the version-specific validator classifies a Statement as voiding only after proving the required voiding verb and StatementRef object, then supplies the parsed target to `StatementCandidate::new_voiding`. The immutable `StoredStatement` retains that target, and `StatementKernel::record_voiding_statement` derives the relation from stored meaning; no caller-selected relation target crosses the domain boundary.
- `migration_recovery`: an `ACCESS EXCLUSIVE` barrier precedes the emptiness snapshot, so a concurrent writer finishes before rollback decides whether removal is safe. Ordered schema lifecycle evidence permits rollback only for an empty pre-release deployment; any tenant binding or learning record makes rollback fail closed. Production evidence recovery requires backup/restore rather than schema teardown.
...
xAPI 2.0 is the canonical target. IEEE 9274.1.1 requires an LRS to process a `2.0` request header as `2.0.0`: `ReceivedXapiVersion` retains the exact validated value, selects `2.0.0` for responses, passes the exact value into the in-memory `IngestionReceipt`, and supplies the canonical `2.0.0` processing surface so a later `2.0.0` replay is not misclassified as a conflict. The PostgreSQL `ingestion_receipt.received_xapi_version` is the durable destination, but its Rust adapter remains unfinished. xAPI 1.0.3 support exists only as an explicit compatibility surface required by cmi5 Quartz. At that compatibility boundary, ADL xAPI 1.0.3 requires `1.0` to be processed as `1.0.0`, otherwise-valid `1.0.x` request labels to be accepted, and every LRS response to use the latest patch version. The parser therefore selects response value `1.0.3` while canonical Statements and the normalized `XapiVersion::V1_0_3` label remain `1.0.0`. It rejects whitespace changes, combined values, unsupported versions, and malformed or leading-zero patches rather than guessing intent. Receipts retain each exact `1.0` or `1.0.x` label. Replay comparison follows the Statement Comparison Requirements of the canonical protocol surface rather than raw request-body equality. Compatibility transformations are owned by `compatibility_adapter`; their outputs remain provenance artifacts linked to received evidence and are not promoted into a second canonical Statement identity.
