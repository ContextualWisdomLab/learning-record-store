...
xAPI 2.0 is the canonical target. IEEE 9274.1.1 requires an LRS to process a `2.0` request header as `2.0.0`: `ReceivedXapiVersion` retains the exact validated value, `ingestion_receipt.received_xapi_version` is the future durable destination for that value, and `statement_record.received_xapi_version` stores canonical `2.0.0` processing semantics so a later `2.0.0` replay is not misclassified as a conflict. xAPI 1.0.3 support exists only as an explicit compatibility surface required by cmi5 Quartz. At that compatibility boundary, ADL xAPI 1.0.3 requires `1.0` to be processed as `1.0.0` and otherwise-valid `1.0.x` request labels to be accepted. The parser rejects whitespace changes, combined values, unsupported versions, and malformed or leading-zero patches rather than guessing intent. Receipts retain each exact `1.0` or `1.0.x` label, while canonical Statements and the Rust kernel's normalized `XapiVersion::V1_0_3` label use `1.0.0`, the stable xAPI 1.0 data-model version. Replay comparison follows the Statement Comparison Requirements of the canonical protocol surface rather than raw request-body equality. Compatibility transformations are owned by `compatibility_adapter`; their outputs remain provenance artifacts linked to received evidence and are not promoted into a second canonical Statement identity.
