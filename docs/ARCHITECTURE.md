...
- `migration_recovery`: ordered schema lifecycle evidence permits rollback only for an empty pre-release deployment; any tenant binding or learning record makes rollback fail closed. Production evidence recovery requires backup/restore rather than schema teardown.
