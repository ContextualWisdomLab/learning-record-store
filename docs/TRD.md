...
`migrations/rollback_statement_evidence.sql` is a pre-release deployment-recovery path, not a data-deletion mechanism. It removes the schema only when every tenant binding and learning-evidence relation is empty, leaves the cluster-level constrained writer role in place, and rolls its refusal back atomically. The hosted lifecycle fixture proves empty rollback, ordered 0001→0004 reapplication, a successful write after reapply, and preservation of retained evidence when a later rollback is refused. Backup/restore and production forward-only migrations remain separate release gates.
