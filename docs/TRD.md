...
`tests/postgres_atomic_ingestion.sh` races identical submissions and competing content against a real PostgreSQL service and requires exactly one canonical row plus two preserved request occurrences, with `accepted`/`replayed` or `accepted`/`conflict` outcomes as appropriate. `tests/postgres_batch_transaction.sh` specifies one-receipt acceptance/replay, conflict-driven whole-batch rejection, canonical non-overwrite, and duplicate fail-closed behavior with durable rejected occurrences. An overlapping item-vs-batch concurrency fixture and measured lock-wait/hot-key latency are still required before read/write splitting or partitioning; no heuristic sharding or weighting is added without evidence.
